...
class ANNSlabSolver(object):
    ...
    def __init__(self, N=4, n_nodes=4, n_points_z=32, z_max=8., sigma_t=1.,
    ...
    def _loss(self, y_pred, z):
        ...
        loss += 0.5 * self.gamma_l * ((y_pred[0, self.N//2:])**2).sum().reshape(1)
        loss += 0.5 * self.gamma_r * ((y_pred[-1, :self.N//2])**2).sum().reshape(1)
